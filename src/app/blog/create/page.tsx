...
export default function blogCreatePage() {
    ...
    const [title, setTitle] = React.useState<string|undefined>("Sample Trip")
    const [description, setDescription] = React.useState<string|undefined>("(Sample) Tokyo racecourse: following the trail of umamusume legends")
    const {
        currentUser,
        loading: userLoading,
        error: userError,
    ...
    const handleFormSubmit = async () => {
        ...
        await apiClient.post("/api/v2/blogs",{
    ...
    return (
        <div className="min-h-screen bg-black relative overflow-hidden flex flex-col justify-center items-center">
            ...
            <div className="w-4/5 flex items-center justify-center border-1 border-white/10 mb-5">
                <SimpleEditor mode="default" onUpdate={handleBlogChange}/>
            </div>
        </div>
    );
}
